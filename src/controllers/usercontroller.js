...
module.exports = {
    //Função responsárvel por buscar todos os usuários que foram cadastrados na aplicação
    async index(request, response) {

        //Método findAll() é utilizado para buscar por todos os dados dos usuários existentes no banco de dados.

        //Utilizamos a propriedade raw com valor true(verdadeiro), porque a função findAll() vai trazer
        //informações desnecessárias, por isso, para retornar somente um array de objetos, passamos essa propriedade.
        const users = await User.findAll({raw: true});

        return response.json(users);
    },

    //Função responsárvel por cadastrar os usuários na aplicação
    ...
    async delete(request, response) {
        ...
        //retornar uma resposta ao formato json().
        return response.json(user);
    }
}